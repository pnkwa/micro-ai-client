...
import { useImageUpload, type UploadItem } from '~/core/composables/useImageUpload'
...
/**
 * The queue and the loop live in `useImageUpload`, shared with dropping files on the grid.
 *
 * The dropzone is not a shortcut past this dialog: same validation, same sequential upload, same
 * naming from the filename, same filing. A second copy of that loop is exactly how the two would
 * drift, and the naming rule in particular is subtle enough that one of them would lose it.
 */
const {
    items,
    isUploading,
...
/**
 * The album, then the loop. Everything about HOW a file is uploaded lives in the composable; what
 * is left here is which album the batch lands in and what to say when it finishes.
 */
const start = handleSubmit(async (values) => {
    await runUploads(values.albumId ?? null)
    done.value = true
